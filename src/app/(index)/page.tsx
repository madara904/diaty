          ...
          <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
            <div className="lg:grid lg:grid-cols-2 lg:gap-28 items-center">
              <div className="sm:text-center lg:text-left">
                <h1 className="text-7xl tracking-tight font-extrabold sm:text-6xl md:text-7xl">
                  <span className="block">Discover</span>
                  <span className="block text-primary">diaty</span>
                </h1>
                <p className="mt-5 text-lg text-muted-foreground sm:mt-6 sm:text-xl sm:max-w-xl sm:mx-auto lg:mx-0">
              ...
              <dl className="space-y-10 md:space-y-0 md:grid md:grid-cols-3 md:gap-x-8 md:gap-y-10">
                {perks.map((perk) => (
                  <div key={perk.title} className="relative">
                    <dt>
                      <div className={cn("absolute flex items-center justify-center h-12 w-12 rounded-md bg-primary text-primary-foreground", perk.color)}>
                        <perk.icon className="h-6 w-6" aria-hidden="true" />
                      </div>
                      <p className="ml-16 text-lg leading-6 font-medium">{perk.title}</p>
                    </dt>
                    <dd className="mt-2 ml-16 text-base text-muted-foreground">{perk.description}</dd>
                  </div>
                ))}
              </dl>
            </div>
